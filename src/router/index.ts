...
router.beforeEach(async (to, from, next) => {
  ...
  if (userStore.isLogin) {
    ...
    if (!['ipLimit'].includes(to.name as string) && to.name !== 'login') {
      next({
        name: 'login',
        query: {
...
router.afterEach((to, from) => {
  ...
  if (to.meta.cache) {
    const componentName = to.matched.at(-1)
    if (componentName) {
  ...
  if (from.meta.cache) {
    const componentName: RouteRecordName | undefined = from.matched.at(-1)?.name
    if (componentName) {
